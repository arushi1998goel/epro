...
public class NewSubjectDetailPage extends BasePage implements CentralRatingModuleConstants {
	...
	@FindBy(xpath = "//div[contains(@class,'selected')]//a[contains(@data-ng-click,'SubjectVisit') and @title='Cancel appointment' and contains(@class,'btn-white ng-hide')]")
	...
	@FindBy(xpath = "//div[contains(@class,'selected')]//a[@title='Edit' and @class='btn circle-button btn-white']")
	...
	@FindBy(xpath = "//div[@id='template']//div[@class='section ng-scope']")
	...
	@FindBy(css = "div[id='virgilForms'] div[class='small'] span")
	...
	@FindBy(xpath = "//div[@class='row administered-row']//button[contains(@class,'btn-default')]//span[1]")
	...
	@FindBy(xpath = "//div[@class='status-editor']")
	...
	@FindBy(xpath = "//div[@data-display-value='changeReason.value']//span[@id='selectedStudy']")
	...
	@FindBy(xpath = "//div[@data-ng-click='cancelClick()']")
	...
	@FindBy(xpath = "//div[@class='list-column']//div[@class='section-header']")
	...
	@FindBy(xpath = "//div[@class='modal-body']//div[@data-ng-click='okClick()']")
	private WebElement approveDeleteMessageFromConfirmationWindow;

	@FindBy(xpath = "//div[@class='modal-body']//div[@data-ng-click='cancelClick()']")
	...
	@FindBy(xpath = "//textarea[@data-ng-model='outcomeStatuses.siteProDisableReason']")
	...
	@FindBy(xpath = "//div[@class='row administered-row']//label")
	...
	/**
	 * Click on submitted paper transcription scale to open the assessment
	 * detail page
	 */
	...
	public void clickOnReportedOutComeButton() {
		...
		// waitAndClick(reportedOutComeEditBTN);
	...
	public void verifyReasonForChangeInpIsHighlightedWithDefaultValue() {
		Assert.assertTrue(
				requiredReasonForChangeINP.isDisplayed() && requiredReasonForChangeINP.getText().length() == 0);
		reportInfo();
	}
	...
	public void userCredentialsFieldIsRequiredAndEmpty() {
		...
		Assert.assertTrue(reasonForChangePasswordINP.getAttribute("class").contains("ng-pristine ng-untouched ng-valid")
	...
	/**
	 * Verify Subject detail page is display and screening is displayed as
	 * expected
	 * 
	...
	/**
	 * Verify subject language field is displayed And also Editable in edit
	 * popup
	 */
	...
	/**
	 * Verify text entered in Visit Notes Text Box under Modal Window Is
	 * Displayed
	 */
	...
	/**
	 * Verify length of text entered in Visit Notes Text Box under Modal Window
	 * Is Displayed
	 */
	...
	public void verifyAddVisitIconIsNotDisplayed() {
		Assert.assertEquals(0,
				driver.findElements(By
						.xpath("//div[contains(@class,'selected')]//a[contains(@data-ng-click,'add') and @title='Add' and @class='circle-button btn btn-white']"))
						.size());
		reportInfo();
	}
	...
	public void verifyScaleAndScaleTypeConfigured(String configuredFormType) {
		...
		if (configuredTemplatesList.size() > 0) {
			for (WebElement scaleRow : configuredTemplatesList) {
				getText(scaleRow.findElement(By.xpath("./div[@class='small ng-binding']")));
	...
	public void verifySubjectOpenedInEditMode() {
		...
		reportInfo();
		Assert.assertTrue(isElementDisplayed(editSubjectPopUp));

		waitForElementPresent(subjectEditICN, DEFAULT_WAIT_4_ELEMENT);
		javascriptButtonClick(subjectEditICN);
		javascripctHilightingElement(editSubjectPopUp);
	...
	public void verifyAppointmentPopUpIsDisplayed() {
		waitForWebElementEnable(appointmentPopUpSelectedDate, 10);
	...
	public void verifyPastDateIsDisabledFromSelectedDate() {
		...
		if (!(date == 1)) {
			for (int dateVar = 0; dateVar < datesList.size() - 2; dateVar++) {
	...
	public void selectAppointmentTime(String timeToSelect) {
		...
		doubleClickOnElement(appointmentPopUpTimeSelectionDROPDOWN);
	...
	public void clickOnYesBtnToCancelTheVisit() {
		...
		new WebDriverWait(driver, 15).until(ExpectedConditions.visibilityOf(cancelAppointmentOKBTN));
	...
	public void verifyVisitCancelledAndRescheduledBtnIsNotDisplayed() {
		Assert.assertFalse(visitCancelHideBTN.isDisplayed() && visitSchedularHIDEBTN.isDisplayed());
		reportInfo();
	}
	...
	public void verifySubjectStatusHistoryContainsDateAndTime(String date) {
		for (WebElement element : subjectStatusHistoryRow) {
			Assert.assertTrue(getText(element.findElement(By.xpath(".//div[contains(@class,'date-col')]//span[@class='ng-binding']"))).
					trim().contains(date));
		}
		reportInfo();
	}
			...
			for (WebElement scaleRow : configuredTemplatesList) {
				if (getText(scaleRow.findElement(By.xpath("./div[@class='small ng-binding']")))
					...
					Assert.assertTrue(getText(scaleRow.findElement(By
							.xpath("./div[3]/div[@class='row administered-row']//button[contains(@class,'btn-default')]//span[1]")))
	...
	public void waitForProcessingVisitToBeCompleted() {
		...
		do {
			...
			waitForElementPresent(scaleGrid, 15);
			waitForElementToBecomeInvisible(By.xpath("//div[@id='modal-fade' and @class='smart-spinner']"));
	...
	public String selectAppointmentDateAndTime(String selectedTime) {
		...
		if (isElementDisplayed(timeSlotGridTitle) && timeSlotGridTitle.getText().contains(differentTimeSlotMessage)) {
			...
			Assert.fail(getText(timeSlotGridTitle));
			...
			new WebDriverWait(driver, 25).until(ExpectedConditions.invisibilityOfElementLocated(
					By.xpath("//div[contains(@class,'modalshow in')]//div[@class='ng-isolate-scope']")));
			if (isElementDisplayed(timeSlotGridTitle) && timeSlotGridTitle.getText().contains(selectDifferentTime)) {
				selectedTime = selectDyanamicAppointmentTimeSlot();
				clickOnYesButtonToConfirmAppointment();
			}
		}
	...
	public String reScheduledVisitAppointmentTime(String selectedTime) {
		selectAppointmentTime(selectedTime);
	...
	/**
	 * Mobile Pro Subject Disable Reason text box is highlighted as required
	 * field
	 */
	...
	/**
	 * Mobile Pro Observer Disable Reason text box is highlighted as required
	 * field
	 */
	...
	/**
	 * Verify text into Mobile Pro Subject Reason text box while its on edit
	 * mode (i.e. value not saved yet)
	 */
	...
	public void verifyBreadCrumbsPanelIsDisplayed() {
		Assert.assertTrue(
				isElementPresent(homeBTN) && isElementPresent(studyProfileLinkICN) && isElementPresent(ratersLinkICN)
	...
	public void verifyPresenceOfQueryBySubject(String Subject) {
		boolean flag = false;
		for (WebElement subjecttoselect : querieList) {
	...
	public void selectFirstUnFlaggedMessageRow() {
		...
		if (sizeofTheList(listOfMessages) > 0) {
			for (WebElement message : listOfMessages) {
				if (message.findElement(By.xpath("./div//span")).getAttribute("class").equalsIgnoreCase("flag set")) {
					continue;
				} else {
					clickOn(message);
					flag = true;
	...
	/**
	 * Verify Messages sent by Observer are indicated with the green 'person'
	 * icon
	 * 
	 */
	...
	public void verifyMessageAreaAfterSelectingMessage() {
		boolean flag = false;
		Assert.assertTrue(messageArea.isDisplayed(), "Message area displayed after selecting message");
		try {
	...
	/**
	 * Verify set / clear Flag Icon Not Displayed On Message Area After
	 * Selecting Message
	 */
	...
	public void verifyIncludeAutomatedCheckedByDefault() {
		...
		Assert.assertTrue(checkBoxIncludeAutomated.isSelected());
	...
	public void verifyReplyButtonDisplayed() {
		...
		Assert.assertTrue(btnReply.isDisplayed());
		moveToElement(btnReply);
		reportInfo();
	}
	...
	/**
	 * Verify option to deactivate the device is displayed for Registered
	 * Subject
	 */
	...
	public void verifyDeActivatedDeviceHistoryWithReasonForChangeDisplayed(String SelectTypeOfEntity) {
		...
		for (WebElement deviceStatus : deviceStatusList) {
			if (deviceStatus.getText().trim().equalsIgnoreCase("Deactivated")) {
				Assert.assertTrue((deviceStatus
						.findElement(By
								.xpath("./ancestor::div[@class='device-wrapper ng-scope']//div[@class='ng-scope inactive']//p[contains(@data-ng-show,'changeReason')]"))
	...
	public void verifyDeActivatedDeviceHistoryWithEsignatureDisplayed(String SelectTypeOfEntity) {
		...
		for (WebElement deviceStatus : deviceStatusList) {
			if (deviceStatus.getText().trim().equalsIgnoreCase("Deactivated")) {
				Assert.assertTrue((deviceStatus
						.findElement(By
								.xpath("./ancestor::div[@class='device-wrapper ng-scope']//div[@class='ng-scope inactive']//p[contains(@data-ng-show,'changedBy')]"))
	...
	public void verifyDateFormatOfDeActivatedDevice(String SelectTypeOfEntity, String datePattern) {
		...
		for (WebElement deviceStatus : deviceStatusList) {
			if (deviceStatus.getText().trim().equalsIgnoreCase("Deactivated")) {
				...
				String date = deviceStatus
						.findElement(By
								.xpath("./ancestor::div[@class='device-wrapper ng-scope']//div[@class='ng-scope inactive']//div[@class='col-date']/p"))
						.getText();
				moveToElement(deviceStatus.findElement(By.xpath(
	...
	/**
	 * Verify Mobile PRO section with Subject and Observer settings is not
	...
	/**
	 * Verify Mobile PRO section with Subject settings is available on the
	 * screen
	 */
	...
	/**
	 * Verify Mobile PRO section with Subject and observer settings is available
	 * on the screen
	 */
	...
	public void clickOnCalenderVisitdDocumentToShow(String documentToBeSelect) {
		clickOn("//div[@class='details-column']//ul[@class='subject-category-filter']//li//a[contains(text(),'"
				+ documentToBeSelect + "')]");
	}
	...
	/**
	 * Verify Unavailability Of Count On Tab Header in Subject category Filter
	 * list section
	 */
	...
	/*
	 * Verify Status(Expired/Completed) Icon Present Under Questionnaires
	 * Details Block
	 */
	...
	public void verifyQuestionnairesInformationUnderDetailsBlock(String detailToBeVerify, String valueToBeVerify) {
		...
		if (detailsToBeVerify.isDisplayed()) {
			...
			Assert.assertEquals(valueToBeVerify, textValueToBeCheck.getText());
			flag = true;
		}
		Assert.assertTrue(flag);
		reportInfo();
	...
	/*
	 * Verify Selected Observer Showing In Reported Outcomes Section In detail
	 * page
	 */
	...
	/*****
	 * verify all the Elements are present in the status Change for Subject
	 * DropDown
	 */
	...
	public void selectTodayDateOnCrSelfCalenderPopUp(String dateToSelect) {
		...
		for (WebElement selfCrdateList : calenderDateList) {
			if (selfCrdateList.isEnabled()) {
				String text = getText(selfCrdateList.findElement(By.xpath(".//span")));
	...
	/**
	 * Select Raters for Calendar Visit from Dropdown for multiple configured
	 * Form Type
	 */
	...
	public void deactivateSubjectConfiguration(String userName, String password) {
		...
		eSignReasonForChangeAndSubmit(userName, password);
		clickOnSubjectResigtrationCrossControl();
	}
	...
	public void deactivateObserverConfiguration(String userName, String password) {
		...
		selectReasonForChangeOption(Constants.reasonsForChangeDeactivateDevice.get(0));
		eSignReasonForChangeAndSubmit(userName, password);
		clickOnObserverResigtrationCrossControl();
	}
