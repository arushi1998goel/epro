package net.medavante.portal.qa.pages.centralrating;
...
public class CentralRatingAppointmentPage extends BasePage {
	...
	// @FindBy(xpath =
	// "//label[@class='title-grey']/a[@data-ng-click='onLoadCalendar()']")
	// private WebElement clinicanLinkForTimeSlot;
	...
	@FindBy(xpath = "//div[@class='row ng-scope']//textarea")
	...
	public void setStartedTime(String hourToBeSelect, String minutesToBeSelected, String timeMarker) {
		/*
		 * if (timePickerPopup.isDisplayed() == false) {
	...
	public void verifyPopUPConfirmationMessageForAppointment(String firstMessage, String middlemessage,
			String lastMessage) {
		String popUpMessage = (firstMessage + middlemessage + lastMessage).toLowerCase();

		Assert.assertTrue((popUpConfirmationMeassage.getText().toLowerCase().contains(popUpMessage)));
	...
	public void selectFurtureDateFromCalender()
	{
		...
		String futuredate=dateCalendarRef.getFutureDate();
		...
		Assert.assertTrue(flag,"Future Date selected ");
	}
		...
		for (int i = 0; i < modelWiindowVisitHistoryRow.size(); i++) {
			...
			if (appointmentTypeToVerify.trim().equalsIgnoreCase(appointmentType)) {
				if (appointmentType.equalsIgnoreCase("Scheduled")) {
					...
					Assert.assertTrue(false, "Status Not Matched");
				}
				flag = true;
			}
		}
	...
	{
		String[] splitString = getText(appointmentDateTimeText).split("\\s");
		System.out.println(splitString[0].toLowerCase());
