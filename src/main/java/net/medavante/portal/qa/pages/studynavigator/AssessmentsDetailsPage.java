...
public class AssessmentsDetailsPage extends BasePage {
	...
	@FindBy(xpath = "//div[@class='row scores-show']//ul[@class='nav nav-tabs']/li")
	...
	@FindBy(xpath = "//div[@class='queries-list']//div[@class='collapsed']//textarea[@class='reply-text ng-pristine ng-valid ng-touched']")
	...
	@FindBy(xpath = "//div[@data-display-value='selectedContext.name']//span[@data-ng-if='::displayField']")
	...
	@FindBy(xpath = "//div[@class='buttons-frame']/button[text()='Yes']")
	private WebElement yesButton;
	
	@FindBy(xpath = "//div[@class='buttons-frame']/button[text()='No']")
	...
	@FindBy(xpath = "//div[@class='subject-general-section']//div[@class='info-row-vertical']/div[@class='info-sub-row']")
	...
	public void verifyAssessmentPageTitle() {
		Assert.assertTrue(assessmentDetailsPageTitle.isDisplayed());
		reportInfo();
	}
	...
	public void verifyPdfStatusTextAndClosedThumbnailImageIsDisplayed(String statusToBeVerifed) {
		...
		Assert.assertTrue(assessmentClosedThumbnailImage.isDisplayed());
		reportInfo();
	}
...
waitAndClick(changeAssesmentEditBox);
	...
	public void verifyUserSelectedDurationIsSaved(String verifiedSelectedDurationTime) {
		...
		if (savedSelectedDurationTimeTXT.getText().contains("hours")) {
			//String[] hourText = savedSelectedDurationTimeTXT.getText().split("hours");
	 ...
	 * Set Time
	 * 
	 * @param startedTimeToSelect
	 *            (Time to be select in HH:MM Format)
	 * 
	 * @param timeMarker
	 *            (Select AM or PM)
	 */
	...
	public void clickOnCancelButton() {
		waitForElementPresent(cancelButton, DEFAULT_WAIT_4_ELEMENT);
	...
	public void clickOnCheckBoxInAssesmentDetailPage() {
		clickOn(checkBoxOnAssesmentDetailsPage);
	}
	...
	public void clickOnYesButtonOfChangeAssesment() {
		moveToElement(yesButton);
		waitAndClick(yesButton);
	}
	
	public void clickOnNoButtonOfChangeAssesment() {
	...
	public void verifyReasonForChangeOptionsPresentInDropDown(List<String> reasonToSelect) {
		...
		for (WebElement reasonChangeValues : reasonForChangeReasonOptionLIST) {
			...
			System.out.println(getText(reasonChangeValues).trim());
		}
	 ...
	 * Audit History page) ======= verify Assessment Date Is Present In Each
	 * Page Of PDF (page footer & Audit History page)
	...
	/*
	 * verify Assessment Date Is Present In Each Page Of PDF of type with
	 * splitted attributes (including page title / page footer / Audit History
	 * page)
	 */
	...
	public void verifyAssessmentDateIsPresentInEachPageOfPDFofTypeWithSplittedAttributes(
		...
		for (pageNo = 1; pageNo <= totalPage; pageNo++) {
			...
			if (auditTextInformation.contains(DateToBeVeriedInPDF)) {
				continue;
			} else {
				flag = false;
				break;
	...
	/**
	 * Verify Certified Copy field contains the corresponding date when the PDF
	 * was generated
	 * 
	 */
	...
	public void selectQuery(String queryName) {
		...
		for (WebElement querytoselect : queriesList) {
			if (querytoselect.findElement(By.xpath(".//div[@class='thread-message ng-binding']")).getText().equalsIgnoreCase(queryName)) {
				flag=true;
	...
	public void selectOptionFromContextDropDownList(String contextOption, String scoreOption) {
		...
		for (WebElement contextOptionlist : contextMenuLIST) {
			if (getText((contextOptionlist)).trim().equalsIgnoreCase(contextOption)) {
				javascriptButtonClick(contextOptionlist);
				break;
			}
			if (scoreSelector.isDisplayed()) {
				waitAndClick(scoreSelector);
				for (WebElement scoreOptionlist : scoreNameLIST) {
					if (getText((scoreOptionlist)).trim().equalsIgnoreCase(scoreOption)) {
						javascriptButtonClick(scoreOptionlist);
						break;
					}
					waitForSpinner(DEFAULT_WAIT_4_ELEMENT);
				}
	...
	/*
	 * Verify after Completing with 'Mark as not administered' the Not
	 * administered checkbox Flag Not set
	 */
	...
	/*
	 * Verify after Completing with 'Mark as not administered' the Not
	 * administered checkbox Flag Is set
	 */
		...
		for (int i = 1; i < assessmentDetailsInfoListUnderVersionSection.size(); i++) {
			...
			if (getText(versionRow.findElement(By.xpath(".//parent::label"))).trim().equalsIgnoreCase(detailLabel)) {
				...
				flag = getText(versionRow.findElement(By.xpath(".//parent::div[contains(@class,'ng-binding')]")))
						.equalsIgnoreCase(detailToBeVerified);
				flag = true;
				break;
			}
		}
		Assert.assertTrue(flag, detailLabel + " Present in version area and" + detailToBeVerified + "value.");
	}

	/*
	 * Verify Option to authorize automatic submission is available In the
	 * Action drop-down
	 */
	...
	/*
	 * Verify Automatic Submission notes Updated After Removal or Confirm
	 * Submission
	 */
	public void verifyUpdatedRecordOnAllowAutomaticSubmissionNotesSection(String recordType, String recordTobeVerify,
		...
		for (int updatedRecordIndex = 0; updatedRecordIndex < notesSectionRecordsList.size(); updatedRecordIndex++) {
			if (versionExist.equalsIgnoreCase("Submiitted")) {
	...
	public void verifyActionButtonIsNotDisplayed() {
		Assert.assertEquals(driver.findElements(By.xpath("//span[text()='Actions']//parent::a//parent::div")).size(),
				0);
		reportInfo();
	}
