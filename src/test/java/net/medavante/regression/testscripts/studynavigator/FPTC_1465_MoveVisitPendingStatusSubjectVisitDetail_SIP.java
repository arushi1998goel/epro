...
public class FPTC_1465_MoveVisitPendingStatusSubjectVisitDetail_SIP extends BaseTest {

	protected String subjectName = "SUBJ_" + generateRandomString(5);
	protected String assessmentName = "ADAS-Cog14 List 2";
	private String completedNonCRVisit, pendingCRVisit;

	@Factory(dataProvider = "Browsers", dataProviderClass = DataProviders.class)
	public FPTC_1465_MoveVisitPendingStatusSubjectVisitDetail_SIP(String browser) {
		super(browser);
	}
	...
	public void getTestData() throws Exception {
		...
		pendingCRVisit = properties.getProperty("visitClinRoNotAssigned");
		...
		reportLog(
				"At least one Visit_2 with 1 ClinRO not assigned Assessment with Pending status on the Subject Visit Details screen for the Subject from Pr.#2, exists");

		subjectDetailPage.clickOnVisitRow(pendingCRVisit);
		subjectDetailPage.clickOnAddVisitIcon();
		subjectDetailPage.verifyVisitStatus(pendingCRVisit, Constants.Pending_Status);
	...
	/**
	 * ====================================================================================================================
	 * Test Case Id: FP-TC-1465 Test Case Name: Verify that option to move Visit
	 * appear in the case when Pending status on the Subject Visit details
	 * screen exists
	 * ====================================================================================================================
	 * 
	...
	@Test(description = "FP-TC-1465_Verify that option to move Visit appear in the case when Pending status on the Subject Visit details screen exists ", groups = {})
	public void FPTC_1465_VerifyMoveVisitPendingStatusSubjectVisitDetail() {
		...
		visitDetaiLPage.verifyVisitIsPresentInChangeToVisitDropDown(pendingCRVisit);

		reportLog("4: Select the Visit Pr.#4 in the Change To filter for the same Subject");
		visitDetaiLPage.selectChangeToVisit(pendingCRVisit);
		...
		reportLog(
				"8.1: Source Visit becomes blank after a successful change. And all assessments are not assigned to any rater");
		...
		reportLog(
				"9.1: 'Subject Status History' is updated and has the latest status in order of visit completion by system change");
		...
		dashBoardPage.navigateToStudyNavigator();
		studyNavigatorDashBoardPage.navigateToAssessmentsListing();
		assessmentDetailPage = studyNavigatorDashBoardPage.selectByVisitAndSubjectName(pendingCRVisit, subjectName);
